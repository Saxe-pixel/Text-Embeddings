...
Reads ``death_dates.json`` (a mapping of QID to death date strings) and
writes ``qid_with_death_clean.txt`` containing only the QIDs whose death
...
DEFAULT_OUT = BASE / "qid_with_death_clean.txt"
...
def main(in_path: Path, out_path: Path) -> None:
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for qid in qids:
            f.write(f"{qid}\n")
    print(f"\u2705 Wrote {len(qids)} clean deaths to {out_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Filter death_dates.json for dates formatted YYYY-MM-DD",
    )
